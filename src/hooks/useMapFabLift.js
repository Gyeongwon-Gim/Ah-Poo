...
function computeFabLift(sheetTop, viewportHeight, defaultBottomPx) {
 ...
 * @returns {{ translateY: number, interactive: boolean }}
 */
...
export function useMapFabLift({
  ...
  const [sheetTops, setSheetTops] = useState({
    search: NO_SHEET_TOP,
    favorites: NO_SHEET_TOP,
    detail: NO_SHEET_TOP,
  });
  const [sheetDragging, setSheetDragging] = useState(false);
  const dragBySourceRef = useRef({
    search: false,
    favorites: false,
    detail: false,
  });
  ...
  const onSearchSheetTopChange = useCallback((top) => {
    setSheetTops((prev) =>
      prev.search === top ? prev : { ...prev, search: top },
    );
  }, []);

  const onFavoritesSheetTopChange = useCallback((top) => {
    setSheetTops((prev) =>
      prev.favorites === top ? prev : { ...prev, favorites: top },
    );
  }, []);

  const onDetailSheetTopChange = useCallback((top) => {
    setSheetTops((prev) =>

...
const SIDO_CANONICAL: readonly (readonly string[])[] = [
  ['서울', '서울특별시', '서울시'],
  ['부산', '부산광역시'],
  ['대구', '대구광역시'],
  ['인천', '인천광역시'],
...
const SIDO_ALIAS = new Map<string, string>();
for (const entry of SIDO_CANONICAL) {
  const canonical = entry[0];
  if (!canonical) continue;
  SIDO_ALIAS.set(canonical, canonical);
  for (const alias of entry.slice(1)) SIDO_ALIAS.set(alias, canonical);
}
...
function districtTokens(tokens: string[]): string[] {
  return tokens.filter((t) => /(시|군|구)$/.test(t));
}

/** 검색 토큰 하나가 수영장과 매칭되는지 판정. */
function tokenMatchesPool(token: string, pool: PoolSearchFields): boolean {
  ...
  if (
    tokens.length > 0 &&
    tokens[0] &&
    normalizeSido(tokens[0]) === normalizeSido(token)
  ) {
    return true;
  }
...
export function poolMatchesQuery(
  pool: PoolSearchFields,
 ...
 */
function tokenScore(token: string, pool: PoolSearchFields): number {
  const lower = token.toLowerCase();
  ...
  if (
    addrTokens.length > 0 &&
    addrTokens[0] &&
    normalizeSido(addrTokens[0]) === normalizeSido(token)
  ) {
    return 20;
  }
...
export function filterBySearchTerm<T extends PoolSearchFields>(
  pools: T[],
  searchTerm: string | null | undefined,
): T[] {

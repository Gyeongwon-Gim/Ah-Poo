 ...
 *
 * @param {object} map - kakao.maps.Map 인스턴스
 * @param {HTMLElement} container - 지도 컨테이너 (재터치 시 관성 취소용)
 * @param {object} [options]
 * @param {number} [options.friction=0.94] - 16ms당 속도 감쇠율 (작을수록 빨리 멈춤)
 * @param {number} [options.minSpeed=2e-7] - 멈춤 임계 속도 (deg/ms)
 * @param {number} [options.maxStartSpeed=0.02] - 관성 시작 속도 상한 (deg/ms, 과도한 튐 방지)
 * @returns {() => void} cleanup
 */
export function attachMapInertia(map, container, options = {}) {
  const { kakao } = window;
  ...
  const clampSpeed = (v) => Math.max(-maxStartSpeed, Math.min(maxStartSpeed, v));

 ...
 */
export function formatPoolFee(fee: string | number | null | undefined): string {
  if (fee == null) return '';
  const s = String(fee).trim();
  if (!s) return '';

  const withCommas = (n: number) => n.toLocaleString('ko-KR');
  const asWon = (digits: string) => {
  ...
  if (currencySuffix?.[1] && /^[\d,\s]+$/.test(currencySuffix[1].trim())) {
...
export function formatDailyAdmissionFee(

...
describe('Explore - 로딩/에러/빈 상태', () => {
  ...
  it('수영장이 0개면 빈 상태 메시지를 보여준다', async () => {
    ...
    renderHome();
    expect(
      await screen.findByText('등록된 수영장이 없습니다'),
    ).toBeInTheDocument();
  });
});
...
describe('Explore - 위치 상태', () => {
  it('위치 확인 중에는 위치 안내 메시지를 보여준다', async () => {
    ...
    renderHome();
    expect(
      await screen.findByText('내 위치를 확인하는 중…'),
    ).toBeInTheDocument();
  });
  ...
  it('위치 권한이 거부되면 권한 안내를 보여준다', async () => {
    ...
    renderHome();
    expect(
      await screen.findByText('위치 권한이 필요합니다'),
    ).toBeInTheDocument();
  });
  ...
  it('위치를 확인할 수 없으면 마커를 표시하지 않는다', async () => {
    ...
    expect(
      await screen.findByText('위치를 확인할 수 없습니다'),
    ).toBeInTheDocument();
  });
...
describe('Explore - 검색', () => {
  it('검색어 제출 시 결과 패널에 필터된 수영장이 표시된다', async () => {
    ...
    expect(within(panel).getByTestId('panel-title')).toHaveTextContent(
      '검색 결과',
    );
  ...
  it('검색 결과가 1건이면 상세 시트를 자동으로 연다', async () => {
    ...
    expect(await screen.findByTestId('detail-sheet')).toHaveTextContent(
      '부산수영장',
    );
  });
});

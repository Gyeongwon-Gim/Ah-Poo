...
export interface BlogItemWithLink {
  link: string;
}

export interface BlogItemWithThumbnail extends BlogItemWithLink {
  thumbnailUrl?: string;
}

/** blog.naver.com/{id}/{logNo} → m.blog.naver.com/{id}/{logNo} */
export function toMobileBlogUrl(link: string): string {
  try {
...
interface FetchBlogThumbnailOptions {
  timeoutMs?: number;
}

export async function fetchBlogThumbnail(
  link: string,
  { timeoutMs = 1200 }: FetchBlogThumbnailOptions = {},
): Promise<string | null> {
...
interface EnrichBlogThumbnailsOptions {
  limit?: number;
}

export async function enrichBlogThumbnails<T extends BlogItemWithLink>(
  items: T[],
  { limit = 5 }: EnrichBlogThumbnailsOptions = {},
): Promise<(T | (T & { thumbnailUrl: string }))[]> {
